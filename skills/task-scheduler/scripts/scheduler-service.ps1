    ...
    try {
        ...
        foreach ($id in $skillTasks.Keys) {
            ...
            $newSchedule = [ordered]@{
                ...
                createdAt      = if ($t.created_at) { [datetimeoffset]::Parse($t.created_at).ToUnixTimeMilliseconds() } else { [datetimeoffset]::UtcNow.ToUnixTimeMilliseconds() }
                lastRunAt      = if ($t.last_run) { [datetimeoffset]::Parse($t.last_run).ToUnixTimeMilliseconds() } else { $null }
        ...
        foreach ($s in $appSchedules) {
            ...
            if ($needsUpdate) {
                ...
                if ($t.last_run) {
                    try { $s.lastRunAt = [datetimeoffset]::Parse($t.last_run).ToUnixTimeMilliseconds() }
    ...
    try {
        ...
        foreach ($file in $taskFiles) {
            try {
                ...
                $nextRun = [datetime]::Parse($task.next_run).ToUniversalTime()
                if ($nextRun -le $now) {
                    $dueTasks += $task
                }
            } catch {
        ...
        if ($dueTasks.Count -gt 0) {
            ...
            foreach ($task in ($dueTasks | Sort-Object -Property next_run)) {
                ...
                if ($freshTask.next_run) {
                    try {
                        $freshNextRun = [datetime]::Parse($freshTask.next_run).ToUniversalTime()
