...
def matches_keyword(text: str, keyword: str) -> bool:
    ...
    '@' is lost (e.g. '@agent' becomes 'agent').  Match both with and without
    the '@' prefix so mentions are always recognized.
    """
    kw = keyword.lower()
    t = text.lower()
    if kw in t:
        return True
    if kw.startswith("@") and kw[1:] in t:
        return True
    return False
...
def extract_prompt(text: str, keyword: str) -> str:
    ...
    t = text.lower()
    idx = t.find(kw)
    if idx == -1 and kw.startswith("@"):
        idx = t.find(kw[1:])
        if idx != -1:
            return text[idx + len(kw) - 1:].strip()
    if idx == -1:
        return text.strip()
...
class MessageHandler:
    ...
    async def handle(self, event: EventMessage) -> None:
        ...
        # 3. Keyword match
        if not matches_keyword(plain_text, self.config.keyword):
            return  # Not for us
        ...
        # --- Passed all filters ---
        prompt = extract_prompt(plain_text, self.config.keyword)
        logger.info(
