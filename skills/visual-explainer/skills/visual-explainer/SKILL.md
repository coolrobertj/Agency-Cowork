...
description: >
  ...
  -- present it as a styled HTML page instead.
  Origin: github.com/nicobailon/visual-explainer (MIT, v0.6.2).
---
...
Detailed prompt templates in `../../commands/`. In Claude Code, namespaced (`/visual-explainer:diff-review`). In Copilot, invoke by describing the workflow (e.g., "generate a visual diff review").
...
- Brand-aligned (uses DESIGN.md — see below)

**Brand-aligned aesthetic (from DESIGN.md):**
Use when the output is stakeholder-facing, the user requests branded output, or consistency with other branded materials matters. If `DESIGN.md` exists at the project root, read it and map its values to CSS custom properties:
- `--bg` ← Background color, `--surface` ← Surface color, `--border` ← derived from Surface at 60% opacity
- `--text` ← Dark color, `--text-dim` ← Muted color
- `--accent-1` ← Primary color, `--accent-2` ← Accent color (with `-dim` variants at 15% opacity)
- `font-family` declarations from Typography Rules (heading → `--font-heading`, body → `--font-body`, monospace → `--font-mono`)

If `DESIGN.md` does not exist, skip this aesthetic and use one of the other constrained options. The existing 4 aesthetics remain preferred for internal/technical diagrams — variation still matters. Brand-aligned is for external-facing or branded consistency.
...
|---|---|---|
| Architecture (any complexity) | **CSS flow diagram** + detail cards | Readable in both light/dark mode; supports rich content; no Mermaid scaling issues |
| Linear flow (3-4 nodes) | CSS flow or simple Mermaid | Either works for trivial topologies |
| Flowchart / pipeline (branching) | **Mermaid** | Decision diamonds and parallel branches need automatic edge routing |
...
**Mermaid vs CSS flow diagrams:** Prefer CSS flow diagrams (bordered boxes + inline SVG arrows) over Mermaid for any diagram where nodes represent components with labels and descriptions. Mermaid's `themeVariables` control fill colors and text colors, but these are baked into the SVG at render time and **do not respond to `prefers-color-scheme`** -- a palette that's readable in light mode becomes invisible in dark mode (or vice versa). CSS flow diagrams use CSS custom properties (`var(--navy)`, `var(--navy-dim)`, etc.) that automatically adapt to both themes. Reserve Mermaid for diagram types that genuinely need automatic edge routing (flowcharts with branches, sequence diagrams, ER diagrams, state machines).

**CSS flow diagram pattern:** Use `<div>` boxes with `border: 2px solid var(--accent)` and `background: var(--accent-dim)` for nodes. Label text uses `color: var(--accent)` with `font-family: var(--font-mono)`. Connect nodes with inline `<svg>` arrows (stroke uses `var(--text-dim)`). Vertical flows use `flex-direction: column`, horizontal flows use `flex-direction: row`. For splits (one parent, multiple children), nest a `display: flex; gap: 24px` row. Example:

```html
<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
  <div style="background:var(--navy-dim);border:2px solid var(--navy);border-radius:10px;padding:12px 28px;text-align:center">
    <div style="font-family:var(--font-mono);font-size:13px;font-weight:600;color:var(--navy)">Node Label</div>
  </div>
  <svg width="20" height="28" viewBox="0 0 20 28"><path d="M10 2v18m-5-5l5 5 5-5" stroke="var(--text-dim)" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>
  <div style="background:var(--gold-dim);border:2px solid var(--gold);border-radius:10px;padding:12px 28px;text-align:center">
    <div style="font-family:var(--font-mono);font-size:13px;font-weight:600;color:var(--gold)">Next Node</div>
    <div style="font-size:11px;color:var(--text-dim);margin-top:2px">Optional subtitle</div>
  </div>
</div>
```

Arrow variants:
- **Vertical down:** `<svg width="20" height="28" viewBox="0 0 20 28"><path d="M10 2v18m-5-5l5 5 5-5" stroke="var(--text-dim)" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>`
- **Horizontal right:** `<svg width="28" height="20" viewBox="0 0 28 20"><path d="M2 10h18m-6-6l6 6-6 6" stroke="var(--text-dim)" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>`
- **Edge label:** Add `<text>` element inside the SVG: `<text x="18" y="14" font-size="9" fill="var(--text-dim)" font-family="var(--font-mono)">label</text>`

Each node should use a **different accent color** from the page palette to visually distinguish components. Pair the border color with its dim variant for the background (e.g., `--navy` border + `--navy-dim` background).
...
**Mermaid scaling:** Mermaid diagrams with 7+ nodes render too small by default, especially with rich labels (`<small>`, `<br/>`, emoji). For architecture and system diagrams, **use CSS flow diagrams instead of Mermaid** -- they scale naturally, support rich content, and work in both light and dark mode. For non-architecture Mermaid diagrams (flowcharts, sequences) with 10-12 nodes, increase `fontSize` in themeVariables to 18-20px and set `INITIAL_ZOOM` to 1.5-1.6. For 15+ elements in any diagram type, split into multiple diagrams.
...
**Output location:** Write to `output/` in the current working directory. Use a descriptive filename based on content: `modem-architecture.html`, `pipeline-flow.html`, `schema-overview.html`. Create the directory if it doesn't exist.

**Open in browser:**
- Windows: `Start-Process output/filename.html`
- macOS: `open output/filename.html`
- Linux: `xdg-open output/filename.html`
...
**Default: use CSS flow diagrams + detail cards.** This is the correct approach for almost all architecture diagrams because:
- CSS custom properties (`var(--navy)`, `var(--text-dim)`) adapt to both light and dark mode automatically
- Mermaid `themeVariables` bake colors into SVG at render time -- they **do not** respond to `prefers-color-scheme`, so a readable light-mode palette becomes invisible in dark mode
- CSS flow nodes can hold subtitles, descriptions, and sub-labels that Mermaid nodes cannot
- No scaling issues -- CSS boxes size to their content naturally

**Architecture diagram pattern:**
1. Build a vertical (or horizontal) stack of bordered `<div>` nodes connected by inline `<svg>` arrows. Each node gets a unique accent color from the page palette (`--navy`, `--gold`, `--teal`, `--plum`, `--red`, `--green`).
2. For splits (one component fans out to multiple children), nest a `display: flex; gap: 24px` row with separate arrow+node pairs.
3. Below the flow diagram, add a `.grid-3` (or `.grid-2`) of `ve-card` components -- one card per major component. Cards get matching `border-top` accents, an emoji icon, a heading, a short description, and a bulleted list of internals.
4. The flow diagram shows topology and data flow; the cards carry all the detail (sub-components, code references, tool lists, badges).

**Horizontal variant (3-4 nodes in a linear chain):** Use `flex-direction: row` with horizontal SVG arrows. Good for identity chains, pipelines, or before/after comparisons. Add a centered subtitle below for annotations (e.g., "auto-loaded every session").

**When to use Mermaid instead:** Only when the diagram has complex branching, cycles, or cross-links that CSS flexbox cannot express cleanly (e.g., a service mesh with bidirectional arrows, a dependency graph with cycles). In those cases, keep the Mermaid diagram to 4-6 abstract nodes with short labels and pair it with CSS Grid detail cards below.

**CSS Grid only (no flow diagram):** Use when the architecture is text-heavy but relationships are obvious or linear (e.g., a pipeline with sequential stages). Sections as rounded cards with colored borders and monospace labels. Vertical flow arrows between sections. The reference template at `../../templates/architecture.html` demonstrates this pattern.
...
> **Note:** Sharing deploys to a public URL. Per Agency Cowork outbound action policy,
...
**Example:**
```bash
bash {{skill_dir}}/scripts/share.sh output/my-diagram.html
