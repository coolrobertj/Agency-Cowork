...
This is the prompt executed by the task scheduler:

#### Step 1: Load State and Rules
...
Use `mail-SearchMessages` with a natural language query:
```
Search my inbox for emails received after [last_run_timestamp].
Show sender, subject, recipients (To and CC), importance level,
whether I've read it, and the email web link. Exclude drafts.
```
...
**If noise match:** Flag the email with category "Noise" using `mail-FlagEmail` or `mail-UpdateMessage`. Log to triage-history.jsonl. Count for summary.

#### Step 4: Tier Matching
...
#### Step 5: Content Analysis & Categorization
...
#### Step 6: Draft Responses (for Urgent + Needs Response)

For emails categorized as Urgent or Needs Response:

1. Use `mail-CreateDraftMessage` to create a draft with:
   - **No recipients** (To/CC/BCC fields left empty)
   - **Subject:** `Re: [original subject]`
   - **Body format:** HTML
   - **Body content:**
     ```html
     <p><strong>TO:</strong> [original sender email]</p>
     <p><strong>CC:</strong> [original CC list]</p>
     <hr/>
     <p>[AI-generated response based on email content and MAIA domain context]</p>
     <br/>
     <p>— Sent by Maia Agent</p>
     ```
2. The response should be:
   - Concise and executive-ready (follow CLAUDE.md communication principles)
   - Data-driven where possible (reference known MAIA milestones, timelines)
   - End with clear next steps or a direct answer to the question asked
3. Log draft message ID to triage-history.jsonl

#### Step 7: Categorize Emails in Outlook

Use `mail-UpdateMessage` to set categories on each processed email:
- Urgent → category "Urgent" (red)
- Needs Response → category "Needs Response" (yellow)
- FYI → category "FYI" (green)
- Archive → category "Auto-Archived" (gray)

**Note:** Moving emails to folders requires the Graph API `/messages/{id}/move` endpoint which is not directly exposed in the MCP. Instead, use **categories** as the primary organization method and document the folder approach as a future enhancement when the MCP supports move operations.

#### Step 8: Create Todo Tasks

For emails categorized as Urgent or Needs Response (if `todo.enabled` in triage-rules.json):

1. Call `python -m scripts.todo_sync --batch <temp_file>` with the categorized emails
2. Each email becomes a Todo task in the "Email Triage" folder with:
   - Subject prefixed with 🔴 (urgent) or 🟡 (needs response)
   - Body containing Outlook deep link and sender/summary
   - Importance: High (urgent) or Normal (needs response)
   - Due date: Today (urgent) or +2 business days (needs response)
3. Dedup by message_id — re-running triage won't create duplicate tasks
4. Tasks sync to the Microsoft Todo app automatically

#### Step 9: Compose Teams Self-Chat Summary

Build the summary message following the template in `templates/teams-summary.md`. Include the Todo task count from `python -m scripts.todo_sync --teams-section`:

```
## 🔴 URGENT (2)

- **Maia 300 Capex Planning — Decision Needed** — from VIP Contact
  Requests budget approval for 16 canary racks by EOD Friday
  📧 [Open in Outlook](https://outlook.office365.com/mail/deeplink/read/{urlEncodedMessageId}?popoutv2=1&version=20260306001.08) | 📝 Draft ready

## 🟡 NEEDS RESPONSE (3)

- **Maia 400 CPU Selection Update** — from Tier 1 Contact
  Asks for your input on Intel DMR-AP vs AMD Florence trade-offs
  📧 [Open in Outlook](https://outlook.office365.com/mail/deeplink/read/{urlEncodedMessageId}?popoutv2=1&version=20260306001.08) | 📝 Draft ready

## 🟢 FYI — Archived (5)

- **Weekly AISC Status** — from AISC-All — Program metrics update, no action needed
  📧 [Open in Outlook](https://outlook.office365.com/mail/deeplink/read/{urlEncodedMessageId}?popoutv2=1&version=20260306001.08)

## 📋 TODO TASKS (5 open)

🔴 2 urgent | 🟡 3 needs response
Manage: `python -m scripts.todo_cli list`

## 🗑️ NOISE FILTERED (12)

Auto-archived: 5 GitHub notifications, 3 newsletters, 2 AKA updates, 2 Planner auto-assigns

---
*Triage run #42 | 22 emails processed | Next run: 9:00 AM PT*
```

#### Step 10: Post to Teams Self-Chat

**CRITICAL SAFETY RULE:** The triage summary MUST ONLY be posted to the user's
self-chat (`48:notes`).  Never resolve the chat dynamically -- always use the
hardcoded chat ID to prevent accidental delivery to group/vendor chats.

1. Call `teams-PostMessage` with **`chatId: "48:notes"`** (hardcoded self-chat ID)
2. Set `contentType: "html"` for rich rendering (bold, links, colors)
3. **NEVER** use `teams-ListChats`, `teams-SearchTeamsMessages`, or any lookup to find the self-chat -- use `48:notes` directly
4. **ASSERTION:** If for any reason the chat ID is not exactly `48:notes`, ABORT the post and alert the user: "Chat ID validation failed -- refusing to post triage summary to non-self chat"
5. **SCHEDULED RUNS:** After posting, log the delivery in triage-state.json with `"delivered_to": "48:notes"` for audit

#### Step 11: Update State

Update `skills/email-triage/cache/triage-state.json`:
- Set `last_run_timestamp` to current time
- Set `last_message_id` to the most recent processed email ID
- Increment `stats` counters
- Increment `total_runs`

Append each processed email to `skills/email-triage/cache/triage-history.jsonl`:
```json
{"timestamp": "2026-03-16T08:30:00Z", "message_id": "AAMk...", "sender": "VIP Contact", "subject": "Maia 300 Capex", "category": "urgent", "action": "draft_created", "draft_id": "AAMk...", "tier": 1}
```
...
Tasks are deduplicated by `message_id` embedded in the task body as `<!-- msg_id:xxx -->`. Running triage multiple times on the same email won't create duplicate tasks.
