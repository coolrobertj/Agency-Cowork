...
def run_triage(
    ...
    # Create reply drafts for actionable items
        ...
        if draft_mode != "never":
            try:
                ...
                draft_results = create_drafts_for_results(
                    result.actionable_items, client=client,
                    profile=profile, dry_run=False,
                )
                result.draft_results = draft_results
                created = sum(1 for d in draft_results if d.get("type") != "error")
                print(f"  {created} draft(s) created")
            except Exception as e:
...
def format_summary(result: TriageResult, verbose: bool = False) -> str:
    ...
    if verbose:
        fyi = [r for r in result.results if r["category"] == "fyi"]
        if fyi:
            lines.append(f"### 🔵 FYI ({len(fyi)} items)")
            for r in fyi[:10]:
    ...
    if result.draft_results:
        created = [d for d in result.draft_results if d.get("type") != "error"]
        if created:
            lines.append(f"\n📝 **{len(created)} draft(s) created** in Drafts folder")

    # ET-1: Prompt injection warnings
    if result.injection_flags:
        lines.append(f"🛡️ **{len(result.injection_flags)} email(s) flagged for prompt injection**")
