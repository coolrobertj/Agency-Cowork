...
| Tool | Status | Notes |
|------|--------|-------|
| QMD (text search) | Installed (v2.1.0) | 4 collections, 38 docs indexed (re-indexed 2026-05-12). memory-flush.ps1 failed on Windows due to /bin/sh invocation; fallback executed on 2026-05-12: `npx qmd update --no-install` updated the text index and `python skills/qmd-memory/scripts/azure-embed.py` refreshed embeddings. Embeddings saved to `skills/qmd-memory/cache/embeddings`. Consider fixing memory-flush.ps1 for Windows. |
