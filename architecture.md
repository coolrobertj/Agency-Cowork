...
### 36. `display: none` on xterm Overlay Scrollbar Kills Mouse Wheel (2026-04-06)

**Root cause:** In `index.css`, the xterm overlay scrollbar was hidden with `display: none !important`
on `.xterm .xterm-scrollable-element > .scrollbar`. In xterm.js v6, the `.xterm-scrollable-element`
is the layer that handles pointer-event hit-testing for mouse input including wheel events. With
`display: none`, the browser removes the element from the layout AND from pointer-event dispatch,
so scroll wheel events fired over the terminal were silently discarded before reaching xterm's
`MouseService` and `ViewportComponent`.

**What made it hard to find:** The terminal rendered correctly and keyboard input worked fine.
Only mouse wheel and text-selection-drag-to-scroll were broken — behaviors that go through the
pointer-event path, not the keyboard path. Adding a debug wheel listener to the container showed
the events reaching the container wrapper but never triggering xterm's internal handlers.

**Fix pattern:** Use `visibility: hidden` instead of `display: none` to hide xterm scrollbar
elements. `visibility: hidden` keeps the element in layout and pointer-event dispatch
(pointer-events still fire; `display: none` removes it entirely). For monitor terminals that
need the scrollbar visible, use `visibility: visible` to restore. Never use `display: none`
on any xterm internal element — prefer opacity, visibility, or height:0 overrides.

**Applies to:** Any CSS customization of xterm.js internals. `display: none` on `.scrollbar`,
`.xterm-viewport`, or other xterm internal elements can silently break mouse input even when
the terminal visually appears correct.
