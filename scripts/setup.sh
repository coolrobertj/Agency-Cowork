...
s_key = 'servers' if 'servers' in existing else 'mcpServers' if 'mcpServers' in existing else 'servers'
...
# Add new servers
added = []
for key, val in new.get('servers', {}).items():
    if key not in existing[s_key]:
        existing[s_key][key] = val
        added.append(key)
# Migrate mcpServers key to servers for VS Code workspace format
if s_key == 'mcpServers':
    existing['servers'] = existing.pop('mcpServers')
    added.append('(migrated key mcpServers -> servers)')
if added or removed:
...
with open(sys.argv[1]) as f:
    cfg = json.load(f)
skey = 'servers' if 'servers' in cfg else 'mcpServers'
...
# Note about legacy global config
if [[ -f "$GLOBAL_MCP_CONFIG" ]]; then
    echo "  [INFO] Workspace MCP config active at .vscode/mcp.json"
...
LAUNCHER
        ...
        # Check workspace .vscode/mcp.json first, then global mcp-config.json
        if [[ -f "$MCP_CONFIG" ]]; then
            python3 -c "
import json, sys
try:
    ...
    with open(mcp_path, 'r') as f:
        cfg = json.load(f)
    # Support both 'servers' (.vscode/mcp.json) and 'mcpServers' (legacy global)
    s_key = 'servers' if 'servers' in cfg else 'mcpServers' if 'mcpServers' in cfg else 'servers'
