...
function Invoke-OneDriveMigration {
    ...
    # Dedup folder name if it already exists (append -2, -3, etc.)
    if (Test-Path $targetBase) {
        $suffix = 2
        while (Test-Path "${targetBase}-${suffix}") { $suffix++ }
        $targetBase = "${targetBase}-${suffix}"
        Write-Warn "Folder already exists in OneDrive, using: $(Split-Path $targetBase -Leaf)"
    }
    ...
    Write-Ok "Pre-flight checks passed ($localFileCount files to migrate)"

    # Create the OneDrive target directory
    ...
    # Source newer -> overwrite; dest newer -> keep; same mtime -> skip
        ...
        foreach ($file in $localFiles) {
            ...
            if (Test-Path $destFile) {
                ...
                if ($file.LastWriteTimeUtc -gt $destInfo.LastWriteTimeUtc) {
                    # Source is newer -- back up dest, copy source
    ...
    if ($copiedCount -gt 0) {
        Write-Ok "Migrated $copiedCount file(s) to OneDrive"
    }
    if ($backedUpCount -gt 0) {
        Write-Warn "Backed up $backedUpCount conflicting file(s) to $backupDir"
    }
    ...
    $sourceCount = (Get-ChildItem $localMemory -Recurse -File -ErrorAction SilentlyContinue | Measure-Object).Count
    $destCount = (Get-ChildItem $targetMemory -Recurse -File -ErrorAction SilentlyContinue | Measure-Object).Count
    if ($sourceCount -gt 0 -and $destCount -lt $sourceCount) {
        Write-Err "File count mismatch: source=$sourceCount, dest=$destCount. Aborting migration."
...
if (-not $DryRun) {
    ...
    if ($qmdInstalled) {
        ...
        $wsMcpPath = Join-Path $ProjectRoot ".vscode" "mcp.json"
