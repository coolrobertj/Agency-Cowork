...
if ($qmdNodeExeEarly -and $qmdJsEarly) {
    ...
    $mcpObj.servers.qmd.command = $qmdNodeExeEarly
    $mcpObj.servers.qmd.args = @($qmdJsEarly, "mcp")
...
if (Test-Path $McpConfig) {
    ...
    try {
        ...
        # Support both "servers" (.vscode/mcp.json) and "mcpServers" (legacy global)
        $sKey = if ($existing.PSObject.Properties["servers"]) { "servers" } else { "mcpServers" }
        if (-not $existing.$sKey) { $existing | Add-Member -NotePropertyName "servers" -NotePropertyValue @{}; $sKey = "servers" }
        ...
        if ($agencyMcpHealthy) {
            ...
            foreach ($authSrv in $authServers) {
                if ($new.servers.PSObject.Properties[$authSrv]) {
                    $new.servers.PSObject.Properties.Remove($authSrv)
                }
            }
        }

        $added = @()
        foreach ($key in $new.servers.PSObject.Properties.Name) {
            if (-not $existing.$sKey.PSObject.Properties[$key]) {
                $existing.$sKey | Add-Member -NotePropertyName $key -NotePropertyValue $new.servers.$key
                $added += $key
            }
        }
        # If existing file used "mcpServers", migrate key to "servers" for VS Code workspace format
        if ($sKey -eq "mcpServers") {
            $migrated = $existing.mcpServers
            $existing.PSObject.Properties.Remove("mcpServers")
            $existing | Add-Member -NotePropertyName "servers" -NotePropertyValue $migrated
        }
        $changed = ($added.Count -gt 0) -or ($removed.Count -gt 0) -or ($sKey -eq "mcpServers")
        if ($changed) {
    ...
    if (-not $agencyMcpHealthy) {
        ...
        foreach ($authSrv in @("teams", "mail", "calendar", "workiq")) {
            if ($mcpObj.servers.PSObject.Properties[$authSrv]) {
                $mcpObj.servers.PSObject.Properties.Remove($authSrv)
            }
        }
        $mcpConfigContent = $mcpObj | ConvertTo-Json -Depth 10
    }
...
# Clean up legacy global MCP config if workspace config is now in place
if ((Test-Path $McpConfig) -and (Test-Path $GlobalMcpConfig)) {
    Write-Host "  [INFO] Workspace MCP config active at .vscode/mcp.json" -ForegroundColor Gray
