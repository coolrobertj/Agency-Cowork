...
# Check workspace .vscode/mcp.json (primary) then global mcp-config.json (fallback)
$wsConfigPath = Join-Path (Get-Location) ".vscode\mcp.json"
$globalConfigPath = Join-Path $env:USERPROFILE ".copilot\mcp-config.json"
$mcpConfigPath = if (Test-Path $wsConfigPath) { $wsConfigPath } elseif (Test-Path $globalConfigPath) { $globalConfigPath } else { $null }
...
if ($mcpConfigPath) {
    try {
        ...
        $sObj = if ($mcpContent.PSObject.Properties["servers"]) { $mcpContent.servers } elseif ($mcpContent.PSObject.Properties["mcpServers"]) { $mcpContent.mcpServers } else { $null }
        if ($sObj) {
    ...
    Write-Host "  [INFO] No MCP config found (checked .vscode/mcp.json and ~/.copilot/mcp-config.json)" -ForegroundColor DarkGray
}
